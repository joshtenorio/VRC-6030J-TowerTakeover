...
std::vector<Point> PurePursuitController::smoothPath(std::vector<Point> path, int runs){
    ...
    pros::lcd::print(7, "smoothing path...");
    ...
    pros::lcd::print(7, "path generated!");
    return newPath;
}
...
Path PurePursuitController::computeVelocities(std::vector<Point> path, float maxVelocity){
    ...
    for(int i = 1; i < path.size() - 1; i++){
        ...
        double bTerm = 0.5 * (squareNum(x2) - 2 * x2 * k1 + squareNum(y2) - squareNum(x3) + 2 * x3 * k1 - squareNum(y3)) / (x3 * k2 - y3 + y2 - x2 * k2);
...
Point PurePursuitController::findLookaheadPoint(std::vector<Point> path, char pathIndex, double lookaheadDistance){
    ...
    for(int i = 1; i < path.size(); i++){
        ...
        pros::lcd::print(6, "calculating intersections and indexes");
        pros::delay(10);
    }
    ...
    for(int i = 0; i < path.size(); i++){
        ...
        //check if current fractional index is larger than the previous fractional index, and check if -1 <= t <= 1
        ...
        pros::lcd::print(6, "looking for first valid intersection");
        pros::delay(3);
    }
    ...
    pros::lcd::print(6, "found lookahead");
...
void PurePursuitController::runPath(char index, float lookAheadDistance, bool backwards){
    ...
    //while robot is x distance away from last point
    ...
    while(distanceToLastPoint >= 3){ //put stop condition here
        ...
        pros::lcd::print(7, "acceleartion computation");
        ...
        pros::lcd::print(4, "leftFF %f, kv %f", leftFeedForwardTerm, kv);
        pros::lcd::print(5, "leftTar %f, rightTar %f", velocityLeftTarget, velocityRightTarget);
        pros::lcd::print(6, "left %f, right %f", leftOutput, rightOutput);
